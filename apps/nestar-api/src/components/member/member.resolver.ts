...
export class MemberResolver {
    ...
    @Roles(MemberType.USER, MemberType.AGENT) // Rollarni tekshiradi metadata
    @UseGuards(RolesGuard) // authoriz chekck
    @Query(() => String)
    ...
    public async updateMember(
        ...
        delete (input as any)._id; // paylodan id ni olib tashlaymiz _id bazada o'zgarmas maydon va mijoz yuborgan ID ga ishonilmaydi
        return await this.memberService.updateMember(memberId, input);
    }

    @UseGuards(WithoutGuard) // if auth => take id, if not => next
    @Query(() => Member)
    public async getMember
        (@Args("memberId") input: string,  // KIMNI ko'rmoqchi (mijozdan)
            @AuthMember('_id') memberId:  // KIM ko'ryapti (tokendan, null bo'lishi mumkin)
                mongoose.ObjectId): Promise<Member> {
    ...
    @Query(() => Members)
    public async getAgents(@Args('input') input: AgentsInquiry, @AuthMember('_id') memberId: mongoose.ObjectId): Promise<Members> {
