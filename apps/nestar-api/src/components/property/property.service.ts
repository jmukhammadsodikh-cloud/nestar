...
export class PropertyService {
    ...
    public async updateProperty(memberId: ObjectId, input: PropertyUpdate): Promise<Property> {
        ...
        if (propertyStatus === PropertyStatus.SOLD) soldAt = moment().toDate(); // royhatga olayapmiz
        else if (propertyStatus === PropertyStatus.DELETE) deletedAt = moment().toDate(); // royhatga olayapmiz
    ...
    public async likeTargetProperty(memberId: ObjectId, likeRefId: ObjectId): Promise<Property> {
        ...
        const modifier: number = await this.likeService.toggleLike(input); // like bor edi => ochir -1 qaytar, yoqmi +1 qaytar
